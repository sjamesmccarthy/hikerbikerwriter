...
interface CreativeWritingDetailProps {
  slug: string;
}

const CreativeWritingDetail: React.FC<CreativeWritingDetailProps> = ({
  slug,
}) => {
  ...
  useEffect(() => {
    async function fetchFieldNote() {
      ...
      try {
        ...
        let url = `/api/creativewriting/${slug}`;
        if (queryString) {
          url += `?${queryString}`;
        }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (updatedFieldNote.slug !== slug) {
          router.push(`/creativewriting/${updatedFieldNote.slug}`);
        }
      } else {
        const errorData = await res.json();
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-gray-500">Loading creative piece...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="max-xl bg-white flex-1">
        ...
        >
          <DialogTitle>Delete Creative Piece</DialogTitle>
          <DialogContent>
            <p>
